...
* The system should offer users the TMDb ID.
...
2. MovieLister uses very simple core libraries, that are the foundation of alot of 
...
### Final Evaluation
1. The program in its final form fulfils and addresses all of the functional and non-functional requirements spectacularly, to a level that above what I was aiming for. It's optimised, functional, and quick, thanks to the advanced multithreaded web requests and requirements. It doesn't match them all to spec, for example the timeout on requests doesn't exist as no request can take that long, each request is a simple web request and the time is only influenced by the content filter requests, not by the request. In short; the program fulfils its functional and non-functional requirements in the best ways possible, however some requirements dont fit the reality of how the program needs to function.

2. MovieLister doesn't have many area's for improvement thanks to it's well planned foundations. It has charm with the addition of a report feature that tears up the report as you submit it (This is done as the scope of the assessment doesn't allow for servers to communicate with the reports.). The next thing would be to directly connect the Buy and Rent locations to their websites with the movie's page, but this would require a lot of work to save the user a few seconds, and it's good to keep from sending a user to another application or site for retention (in the case of addings ADs). Advertising could be a good feature, where streaming services pay us to have a "recommened" movie list that their movies would go on, or to have them appear as the recommened avenue for viewing a movie (directing more users to their location while having us profit).

3. I think MovieLister as a whole was managed quite well throughout it's whole production, if anything I'd say it was over planned, as most of the planning seemed a bit redundant with all the type of diagrams, especially the Data Dictionary with its actual usages being low considering MovieLister does not register things to its own database or storage. The main challenge was speed, the program was slow at the beginning as it was slowly getting each image from TMDb's site 1 by 1, waiting to run the next one. This was fixed with a not so simple multithreading solution. It was this multithreading that allows the program to be as quick as it is, running all image requests simulataniously (or in batches depending on how many threads python can take.) and filling in images for placeholders means that even when web-requests fail or take forever the program can still function to a base level. I think that the Multithreading solution as it stands while not perfect when it comes to optimisations on low-end systems, it still performs in a way that matches the functional requirements and overcomes the original issue of speed.